...
namespace SingletonPattern
{
    public class GameManager : Singleton<GameManager>
    {
        ...
        public override void Awake()
        {
            ...
            if (startBtn)
                startBtn.GetComponent<Button>().onClick.AddListener(OnClickStartBtn);
            if (optionBtn)
                optionBtn.GetComponent<Button>().onClick.AddListener(OnClickOptionBtn);
            if (gameQuitBtn)
                gameQuitBtn.GetComponent<Button>().onClick.AddListener(OnClickGameQuitBtn);
        }
        ...
        private IEnumerator DoLoading()
        {
            switch (startFrom)
            {
                ...
                case StartFrom.Story:
                    ...
                    _storyManager.storyWindow.SetActive(true);
                    _storyManager.SetStoryNumber(storyNumber);
                    _storyManager.SetIsAvailableProgress(true);
                    _storyManager.DoStoryProgress();
                    break;
                case StartFrom.Game1:
                    _shootingGameManager.StartShootingGame();
                    break;
            }
        }
        ...
        private IEnumerator DoOpening()
        {
            ...
            gameQuitBtn.SetActive(false);
            optionWindow.SetActive(false);
            _storyManager.storyWindow.SetActive(false);
        ...
        {
            _storyManager.storyWindow.SetActive(false);
            _storyManager.storyWidget.SetActive(false);
            _audioManager.PlayBGM("Restart");
            PlayerPrefs.SetInt("StageProgress", storyNum);
        ...
        {
            _shootingGameManager.StopShootingGame();
        }
