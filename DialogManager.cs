...
public class DialogManager : MonoBehaviour
{
    ...
    public void InitializeDialog()
    {
        ...
        else if (LocalizeManager.language == LocalizeManager.Language.Japanese)
        {
            ...
            _storyManager.AppendDialog("", "", "", 1f); // 45
            _storyManager.AppendDialog("???", "", "용사여, 당신을 기다리고 있었습니다.", 1f); // 46
            _storyManager.AppendDialog("ミドリエル", "女神", "제 이름은, 여신 'ミドリエル'.", 1f); // 47
            _storyManager.AppendDialog("", "", "", 1f); // 48
            _storyManager.AppendDialog("ミドリエル", "女神", "우리의 세계 '밀레니엄 랜드'는, 지금 미증유의 위기에 처했습니다.", 1f); // 49
            _storyManager.AppendDialog("ミドリエル", "女神", "이 위기를 극복하고...", 1f); // 50
            _storyManager.AppendDialog("ミドリエル", "女神", "극복하고...", 1f); // 51
            _storyManager.AppendDialog("", "", "", 1f); // 52
            _storyManager.AppendDialog("ミドリエル", "女神", "이 다음에 뭐라고 얘기하면 좋을까...", 1f); // 53
            _storyManager.AppendDialog("", "", "", 1f); // 54
            _storyManager.AppendDialog("???", "", "앗! 정신이 드셨군요!", 1f); // 55
            _storyManager.AppendDialog("", "", "", 1f); // 56
            _storyManager.AppendDialog("???", "", "헉, 이게 아니었지.", 1f); // 57
            _storyManager.AppendDialog("???", "", "크흠!", 1f); // 58
            _storyManager.AppendDialog("???", "", "요, 용사여. 당신을 기다리고 있었습니다.", 1f); // 59
            _storyManager.AppendDialog("ミドリエル", "女神", "제 이름은, 여신 'ミドリエル'.", 1f); // 60
            _storyManager.AppendDialog("ミドリエル", "女神", "우리의 세계 '밀레니엄 랜드'는, 지금 미증류의 위기에 처했습니다.", 1f); // 61
            _storyManager.AppendDialog("", "", "", 1f); // 62
            _storyManager.AppendDialog("ミドリエル", "女神", "앗, 맞다. 으아아… 죄송해요. 제가 사람 앞에서 이야기하는 걸 잘 못해서……", 1f); // 63
            _storyManager.AppendDialog("", "", "", 1f); // 64
            _storyManager.AppendDialog("ミドリエル", "女神", "이해해주셔서 감사해요. 아무튼. 크흠!", 1f); // 65
            _storyManager.AppendDialog("ミドリエル", "女神", "마왕 모모리아로부터 빼앗긴 땅을 되찾고, 밀레니엄 랜드를 구원할 수 있는 자는, 오직 당신뿐입니다.", 1f); // 66
            _storyManager.AppendDialog("", "", "", 1f); // 67
            _storyManager.AppendDialog("ミドリエル", "女神", "괜찮아요! 저희 밀레니엄 랜드에서는, 싸움이 아니라 게임으로 모든 걸 결정하거든요.", 1f); // 68
            _storyManager.AppendDialog("ミドリエル", "女神", "네? 어째서 게임으로 모든 걸 결정하냐고요?", 1f); // 69
            _storyManager.AppendDialog("ミドリエル", "女神", "그건 말이죠. 저희가 게임을 정말 좋아하기 때문이에요.", 1f); // 70
            _storyManager.AppendDialog("ミドリエル", "女神", "물론 게임 때문에 싸우게 되는 일도 많지만요…….", 1f); // 71
            _storyManager.AppendDialog("ミドリエル", "女神", "용사님께서는, 저를 도와주실 수 있으실까요?", 1f); // 72
            _storyManager.AppendDialog("ミドリエル", "女神", "물론, 갑자기 이런 말씀을 드리는 건 예의가 아니지만, 그래도……", 1f); // 73
            _storyManager.AppendDialog("", "", "", 1f); // 74
            _storyManager.AppendDialog("ミドリエル", "女神", "정말요? 감사합니다!", 1f); // 75
            _storyManager.AppendDialog("ミドリエル", "女神", "그럼 어서 첫 번째 신전으로 이동하도록 하죠!", 1f); // 76
            _storyManager.AppendDialog("", "", "", 1f); // 77
            _storyManager.AppendDialog("ミドリエル", "女神", "여기가 마왕 모모리아의 첫 번째 신전이에요!", 1f); // 78
            _storyManager.AppendDialog("", "", "", 1f); // 79
            _storyManager.AppendDialog("ミドリエル", "女神", "저기 보이는 저 게임기가 제가 언니……", 1f); // 80
            _storyManager.AppendDialog("ミドリエル", "女神", "아, 아니, 마왕 모모리아한테 빼앗긴 땅이에요!", 1f); // 81
            _storyManager.AppendDialog("", "", "", 1f); // 82
            _storyManager.AppendDialog("ミドリエル", "女神", "앗, 이건 조금 부끄럽네요.", 1f); // 83
            _storyManager.AppendDialog("ミドリエル", "女神", "사실은 1위를 하려고 수차례 도전을 했지만……", 1f); // 84
            _storyManager.AppendDialog("ミドリエル", "女神", "아쉽게도 보시는 대로에요.", 1f); // 85
            _storyManager.AppendDialog("", "", "", 1f); // 86
            _storyManager.AppendDialog("ミドリエル", "女神", "네, 맞아요. 실은 처음엔 하이 스코어에 가까운 점수를 냈지만, 플레이를 거듭할수록 점점 랭킹에 점수를 올리지조차 못하게 되었거든요.", 1f); // 87
            _storyManager.AppendDialog("ミドリエル", "女神", "뭔가 빠뜨린 게 있는 것 같은데, 저로서는 알 수 없었어요.", 1f); // 88
            _storyManager.AppendDialog("ミドリエル", "女神", "용사님께서 한 번 봐주시겠어요?", 1f); // 89
            _storyManager.AppendDialog("", "", "", 1f); // 90
            _storyManager.AppendDialog("", "", "< 승리! >", 1f); // 91
            _storyManager.AppendDialog("", "", "< 최종 점수: 41,600점 >\r\n< 랭킹 진입 실패! >", 1f); // 92
            _storyManager.AppendDialog("ミドリエル", "女神", "역시나 이번에도…….", 1f); // 93
            _storyManager.AppendDialog("", "", "", 1f); // 94
            _storyManager.AppendDialog("ミドリエル", "女神", "점점 실력이 늘어가는 것을 저 스스로도 느끼는데, 점수는 그렇지가 않더라고요.", 1f); // 95
            _storyManager.AppendDialog("", "", "", 1f); // 96
            _storyManager.AppendDialog("ミドリエル", "女神", "그럼요. 자리 비켜드릴게요.", 1f); // 97
            _storyManager.AppendDialog("", "", "[조작키 안내]\r\n이동: 방향키, 공격: Z, 스킬: X", 1f); // 98
            _storyManager.AppendDialog("", "", "", 1f); // 99
            _storyManager.AppendDialog("", "", "", 1f); // 100
            _storyManager.AppendDialog("ミドリエル", "女神", "적한테 죽어버리다니, 아쉽네요.", 1f); // 101
            _storyManager.AppendDialog("ミドリエル", "女神", "하, 하지만 다음번엔 분명 잘 하실 거예요!", 1f); // 102
            _storyManager.AppendDialog("", "", "", 1f); // 103
            _storyManager.AppendDialog("ミドリエル", "女神", "보스를 쓰러뜨리셨군요! 대단해요!", 1f); // 104
            _storyManager.AppendDialog("ミドリエル", "女神", "그렇지만 아쉽게도 최고 점수에 도달하지는 못했네요.", 1f); // 105
            _storyManager.AppendDialog("ミドリエル", "女神", "옆에서 지켜보니 뭔가 알 것도 같은데……", 1f); // 106
            _storyManager.AppendDialog("ミドリエル", "女神", "다음 번에는 최고 점수를 노려봐요!", 1f); // 107
            _storyManager.AppendDialog("", "", "", 1f); // 108
            _storyManager.AppendDialog("", "", "< 하이 스코어 갱신! >", 1f); // 109
            _storyManager.AppendDialog("ミドリエル", "女神", "우와! 마침내 하이 스코어 갱신이에요!", 1f); // 110
            _storyManager.AppendDialog("ミドリエル", "女神", "처음에 플레이했을 때 가장 높은 점수를 받은 이유가 있었네요.", 1f); // 111
            _storyManager.AppendDialog("ミドリエル", "女神", "실력이 늘수록 클리어 타임이 줄어들고, 그 때문에 하이 스코어를 갱신할 수 없었다니……", 1f); // 112
            _storyManager.AppendDialog("ミドリエル", "女神", "감사합니다. 용사님이 아니었다면 줄곧 모르는 채였을 거예요.", 1f); // 113
            _storyManager.AppendDialog("", "", "", 1f); // 114
            _storyManager.AppendDialog("ミドリエル", "女神", "……", 1f); // 115
            _storyManager.AppendDialog("ミドリエル", "女神", "그렇게 말씀해 주시다니, 용사님은 정말 상냥하신 분이네요.", 1f); // 116
            _storyManager.AppendDialog("ミドリエル", "女神", "언니도 이렇게 상냥했었더라면……", 1f); // 117
            _storyManager.AppendDialog("", "", "", 1f); // 118
            _storyManager.AppendDialog("ミドリエル", "女神", "앗!", 1f); // 119
            _storyManager.AppendDialog("ミドリエル", "女神", "……", 1f); // 120
            _storyManager.AppendDialog("ミドリエル", "女神", "사실, 제가 말했던 마왕 모모리아는 제 언니에요.", 1f); // 121
            _storyManager.AppendDialog("ミドリエル", "女神", "미리 말씀드리지 못해서 죄송해요.", 1f); // 122
            _storyManager.AppendDialog("", "", "", 1f); // 123
            _storyManager.AppendDialog("ミドリエル", "女神", "여, 역시 조금 이상하죠?", 1f); // 124
            _storyManager.AppendDialog("ミドリエル", "女神", "그게 말이죠, 실은……", 1f); // 125
            _storyManager.AppendDialog("", "", "", 1f); // 126
            _storyManager.AppendDialog("ミドリエル", "女神", "네? 음…… 아무래도 그렇죠.", 1f); // 127
            _storyManager.AppendDialog("ミドリエル", "女神", "저도 언니도, 지는 걸 굉장히 싫어해서 같이 게임을 하다 보면 어느샌가 다투고 있곤 해요.", 1f); // 128
            _storyManager.AppendDialog("ミドリエル", "女神", "그리고 언니는, 게임에 몰두하다 보면 하루 종일 게임을 하곤 하거든요.", 1f); // 129
            _storyManager.AppendDialog("ミドリエル", "女神", "옆에서 지켜보다 보면 저도 하고 싶어지지만, 언니에게 얘기를 꺼내기가 어려워서 계속 지켜보다가 울음을 터트리곤 해요.", 1f); // 130
            _storyManager.AppendDialog("ミドリエル", "女神", "앗, 죄송해요. 저도 모르게 부끄러운 얘기를 해버렸네요.", 1f); // 131
            _storyManager.AppendDialog("", "", "", 1f); // 132
            _storyManager.AppendDialog("", "", "", 1f); // 133
            _storyManager.AppendDialog("ミドリエル", "女神", "아, 확실히 그렇긴 하죠.", 1f); // 134
            _storyManager.AppendDialog("", "", "", 1f); // 135
            _storyManager.AppendDialog("ミドリエル", "女神", "음…… 듣고보니 확실히.", 1f); // 136
            _storyManager.AppendDialog("ミドリエル", "女神", "방금 그 게임도 만약 언니가 하는 것을 지켜보았더라면, 진작에 이유를 눈치챘을 지도 모르겠네요.", 1f); // 137
            _storyManager.AppendDialog("", "", "< 랭킹 등록 제한 시간이 20초 남았습니다. >", 1f); // 138
            _storyManager.AppendDialog("ミドリエル", "女神", "헉! 모처럼의 하이 스코어인데, 랭킹 등록을 놓치겠어요! 어서 등록하세요, 용사님!", 1f); // 139
            _storyManager.AppendDialog("", "", "", 1f); // 140
            _storyManager.AppendDialog("", "", "< 최대 6글자까지 입력이 가능합니다. >", 1f); // 141
            _storyManager.AppendDialog("", "", "", 1f); // 142
            _storyManager.AppendDialog("ミドリエル", "女神", "어라, 어째서 제 이름을……", 1f); // 143
            _storyManager.AppendDialog("", "", "", 1f); // 144
            _storyManager.AppendDialog("ミドリエル", "女神", "……", 1f); // 145
            _storyManager.AppendDialog("ミドリエル", "女神", "으아아아앙!!!", 1f); // 146
            _storyManager.AppendDialog("", "", "", 1f); // 147
            _storyManager.AppendDialog("ミドリエル", "女神", "몇 날 며칠을 도전해도 이룰 수 없던 랭킹 1위를 달성하다니……", 1f); // 148
            _storyManager.AppendDialog("ミドリエル", "女神", "정말 감사해요. 모두 용사님 덕분이에요. 흑……", 1f); // 149
            _storyManager.AppendDialog("ミドリエル", "女神", "앞으로도 용사님과 함께라면 뭐든 잘 해낼 수 있을 것 같은 기분이 들어요.", 1f); // 150
            _storyManager.AppendDialog("ミドリエル", "女神", "자, 어서 다음 신전으로 이동하죠!", 1f); // 151
            _storyManager.AppendDialog("", "", "(우는 얼굴을 들키기 싫었는지 서둘리 앞서가는 미도리엘을 뒤따라갔다.)", 1f); // 152
            _storyManager.AppendDialog("", "", "", 1f); // 153
                                                        // 2장 종료
        }
